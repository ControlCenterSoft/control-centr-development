...
Этот слой не является новым источником истины и не создаёт отдельную Incident-модель. Он читает canonical `incidents.Reader`, полностью собирает ограниченный keyset-pagination набор и только затем использует существующий `BuildIncidentOperationalReport`.
...
- сохранять правило существующего adapter: resolved Incident становится `unknown`, потому что операторское закрытие не доказывает восстановление фактического health.
...
При последующем включении этого provider в `/api/v1/ui/reports/resources` и `/reports/resources` сохраняется существующая глобальная граница `resources.read`. Site-scoped identity не должна получать глобальный aggregate через этот route.
...
Код provider и negative/unit test coverage подготовлены отдельно от текущего Incident browser PR и не меняют Incident mutation API. Следующий integration step — передать provider, построенный на canonical PostgreSQL `IncidentReadRepository`, в существующий `withResourceReportsProvider` normal-runtime wiring.
