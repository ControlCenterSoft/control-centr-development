...
// expired, duplicate or cross-current-session evidence rather than presenting
// an incomplete session inventory as authoritative.
func BuildSessionSecurityWorkspace(input SessionSecurityWorkspaceInput) (SessionSecurityWorkspace, error) {
	...
	for _, raw := range input.Sessions {
		view, err := normalizeSessionSecuritySession(raw, now)
		if err != nil {
			return SessionSecurityWorkspace{}, err
		}
...
func normalizeSessionSecuritySession(input SessionSecurityInput, now time.Time) (SessionSecuritySessionView, error) {
