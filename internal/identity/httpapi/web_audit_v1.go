...
<style>
:root{font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color-scheme:light dark;--s1:.35rem;--s2:.65rem;--s3:1rem;--s4:1.5rem;--radius:.8rem;--border:1px solid currentColor}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:Canvas;color:CanvasText}a{color:inherit}.skip{position:absolute;left:-9999px}.skip:focus{left:var(--s3);top:var(--s3);z-index:10;background:Canvas;padding:var(--s2);border:var(--border)}.topbar{display:flex;gap:var(--s3);align-items:center;justify-content:space-between;padding:var(--s3) var(--s4);border-bottom:var(--border)}.brand{display:grid;gap:var(--s1)}.muted,.eyebrow{font-size:.85rem;opacity:.78}.row{display:flex;gap:var(--s2);align-items:center;flex-wrap:wrap}.badge{display:inline-flex;border:var(--border);border-radius:999px;padding:.35rem .65rem;font-size:.85rem}.content{padding:var(--s4);max-width:92rem;width:100%;margin:0 auto}.filters{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:var(--s3);padding:var(--s3);border:var(--border);border-radius:var(--radius)}label{display:grid;gap:var(--s1);font-weight:600}input,button{font:inherit;padding:.55rem .65rem;border:var(--border);border-radius:.55rem;background:Canvas;color:CanvasText;min-width:0}.actions{display:flex;gap:var(--s2);align-items:end;flex-wrap:wrap}.table-wrap{overflow:auto;margin-top:var(--s4);border:var(--border);border-radius:var(--radius)}table{width:100%;border-collapse:collapse;min-width:72rem}th,td{text-align:left;vertical-align:top;padding:.7rem;border-bottom:var(--border);overflow-wrap:anywhere}th{font-size:.85rem}.empty{padding:var(--s3)}.pager,.footer{margin-top:var(--s4);display:flex;gap:var(--s3);justify-content:space-between;align-items:center;flex-wrap:wrap}.hash{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:.8rem}@media(max-width:850px){.filters{grid-template-columns:1fr}.topbar{align-items:flex-start;flex-direction:column}.content{padding:var(--s3)}}
</style>
</head>
<body>
