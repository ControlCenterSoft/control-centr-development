...
[[ "$(tr -d '\r\n' < VERSION)" == "$candidate_version" ]] || { echo "source VERSION must match the exact promoted 0.32 candidate version" >&2; exit 2; }
...
for doc in \
  docs/RELEASE_0.32.0_RU.md \
  docs/RELEASE_CANDIDATE_READINESS_0.32_RU.md \
  docs/QUALIFICATION_0.32_UPGRADE_FROM_0.31_RU.md \
  docs/CC-032-HEALTH-OVERVIEW-READMODEL.md \
  docs/CC-032-INCIDENT-PERSISTENCE.md \
  docs/AUDIT_EXPORT_0_32_RU.md \
  docs/CC-032-REPORTS-EVIDENCE-DRAWER.md; do
  [[ -f "$doc" ]] && cp "$doc" "$stage/$bundle/docs/"
done

# Third-party evidence is immutable release input. 0.32 currently introduces no
# new runtime dependency compared with 0.31, but the manifest is rebound to the
# 0.32 product identity so dependency/license evidence cannot be reused across
